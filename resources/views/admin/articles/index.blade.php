                    ...
                    <form method="GET" action="{{ route('admin.articles.index') }}">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 items-center">
                            ...
                            <div class="flex items-center space-x-2 space-x-reverse">
                                ...
                                <a href="{{ route('admin.articles.index') }}" class="w-full flex items-center justify-center gap-2 px-4 py-2 bg-gray-600/50 text-white rounded-lg hover:bg-gray-500/50 transition" title="{{ __('messages.reset_filters') }}">
                        ...
                        <table class="min-w-full">
                            <thead class="border-b border-white/20">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 {{$marginClass}} text-xs font-medium text-gray-300 uppercase tracking-wider">{{ __('messages.category') }}</th>
                                    <th class="px-6 py-3 {{$marginClass}} text-xs font-medium text-gray-300 uppercase tracking-wider">{{ __('messages.status') }}</th>
                                    <th class="px-6 py-3 {{$marginClass}} text-xs font-medium text-gray-300 uppercase tracking-wider">{{ __('messages.statistics') }}</th>
                            ...
                            <tbody class="divide-y divide-white/10">
                                    ...
                                    <tr class="hover:bg-white/5 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                                ...
                                                {{ $article->status == 'published' ? __('messages.published') : __('messages.draft') }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                            <span class="mr-4" title="{{ __('messages.views') }}"><i class="fas fa-eye mr-1"></i> {{ $article->views_count }}</span>
                                            <span title="{{ __('messages.likes') }}"><i class="fas fa-heart text-red-400 mr-1"></i> {{ $article->likes_count }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap {{$marginClass}} text-sm font-medium">
                                            <div class="flex items-center justify-center gap-4">
                                                <a href="{{ route('admin.articles.show', $article) }}" class="text-green-400 hover:text-green-300" title="{{ __('messages.view_details') }}">
                                                    <i class="fas fa-eye fa-lg"></i>
                                                </a>
                                                <a href="{{ route('admin.articles.edit', $article) }}" class="text-blue-400 hover:text-blue-300" title="{{ __('messages.edit_article') }}">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-10 whitespace-nowrap text-sm text-gray-400 {{$marginClass}}">
